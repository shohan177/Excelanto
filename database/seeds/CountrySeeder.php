<?php

use Illuminate\Database\Seeder;
use App\Country;

class CountrySeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {

        Country::create([
            'id' => 2,
            'country_code' => 355,
            'country_name' => 'Albania(+355)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 3,
            'country_code' => 213,
            'country_name' => 'Algeria (+213)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 4,
            'country_code' => 684,
            'country_name' => 'American Samoa(+684)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 5,
            'country_code' => 376,
            'country_name' => 'Andorra (+376)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 6,
            'country_code' => 244,
            'country_name' => 'Angola (+244)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 7,
            'country_code' => 1264,
            'country_name' => 'Anguilla (+1264)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 8,
            'country_code' => 672,
            'country_name' => 'Antarctica(+672)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 9,
            'country_code' => 1268,
            'country_name' => 'Antigua & Barbuda (+1268)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 10,
            'country_code' => 54,
            'country_name' => 'Argentina (+54)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 11,
            'country_code' => 374,
            'country_name' => 'Armenia (+374)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 12,
            'country_code' => 297,
            'country_name' => 'Aruba (+297)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 13,
            'country_code' => 61,
            'country_name' => 'Australia (+61)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 14,
            'country_code' => 43,
            'country_name' => 'Austria (+43)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 15,
            'country_code' => 994,
            'country_name' => 'Azerbaijan (+994)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 16,
            'country_code' => 1242,
            'country_name' => 'Bahamas (+1242)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 17,
            'country_code' => 973,
            'country_name' => 'Bahrain (+973)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 18,
            'country_code' => 880,
            'country_name' => 'Bangladesh (+880)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 19,
            'country_code' => 1246,
            'country_name' => 'Barbados (+1246)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 20,
            'country_code' => 375,
            'country_name' => 'Belarus (+375)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 21,
            'country_code' => 32,
            'country_name' => 'Belgium (+32)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 22,
            'country_code' => 501,
            'country_name' => 'Belize (+501)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 23,
            'country_code' => 229,
            'country_name' => 'Benin (+229)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 24,
            'country_code' => 1441,
            'country_name' => 'Bermuda (+1441)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 25,
            'country_code' => 975,
            'country_name' => 'Bhutan (+975)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 26,
            'country_code' => 591,
            'country_name' => 'Bolivia (+591)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 27,
            'country_code' => 387,
            'country_name' => 'Bosnia Herzegovina (+387)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 28,
            'country_code' => 267,
            'country_name' => 'Botswana (+267)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 29,
            'country_code' => 55,
            'country_name' => 'Bouvet Island(+55)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 30,
            'country_code' => 55,
            'country_name' => 'Brazil (+55)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 31,
            'country_code' => 246,
            'country_name' => 'British Indian Ocean Territory(+246)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 32,
            'country_code' => 673,
            'country_name' => 'Brunei (+673)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 33,
            'country_code' => 359,
            'country_name' => 'Bulgaria (+359)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 34,
            'country_code' => 226,
            'country_name' => 'Burkina Faso (+226)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 35,
            'country_code' => 257,
            'country_name' => 'Burundi (+257)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 36,
            'country_code' => 855,
            'country_name' => 'Cambodia (+855)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 37,
            'country_code' => 237,
            'country_name' => 'Cameroon (+237)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 38,
            'country_code' => 1,
            'country_name' => 'Canada (+1)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 39,
            'country_code' => 238,
            'country_name' => 'Cape Verde Islands (+238)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 40,
            'country_code' => 1345,
            'country_name' => 'Cayman Islands (+1345)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 41,
            'country_code' => 236,
            'country_name' => 'Central African Republic (+236)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 42,
            'country_code' => 235,
            'country_name' => 'Chad(+235)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 43,
            'country_code' => 56,
            'country_name' => 'Chile (+56)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 44,
            'country_code' => 86,
            'country_name' => 'China (+86)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 45,
            'country_code' => 61,
            'country_name' => 'Christmas Island(+61)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 46,
            'country_code' => 61,
            'country_name' => 'Cocos (Keeling) Islands(+61)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 47,
            'country_code' => 57,
            'country_name' => 'Colombia (+57)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 48,
            'country_code' => 269,
            'country_name' => 'Comoros (+269)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 49,
            'country_code' => 242,
            'country_name' => 'Congo (+242)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 50,
            'country_code' => 243,
            'country_name' => 'Congo The Democratic Republic Of The(+243)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 51,
            'country_code' => 682,
            'country_name' => 'Cook Islands (+682)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 52,
            'country_code' => 506,
            'country_name' => 'Costa Rica (+506)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 53,
            'country_code' => 225,
            'country_name' => 'Cote D\'Ivoire (Ivory Coast)(+225)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 54,
            'country_code' => 385,
            'country_name' => 'Croatia (+385)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 55,
            'country_code' => 53,
            'country_name' => 'Cuba (+53)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 56,
            'country_code' => 90392,
            'country_name' => 'Cyprus North (+90392)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 57,
            'country_code' => 42,
            'country_name' => 'Czech Republic (+42)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 58,
            'country_code' => 45,
            'country_name' => 'Denmark (+45)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 59,
            'country_code' => 253,
            'country_name' => 'Djibouti (+253)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 60,
            'country_code' => 1809,
            'country_name' => 'Dominica (+1809)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 61,
            'country_code' => 1809,
            'country_name' => 'Dominican Republic (+1809)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 62,
            'country_code' => 670,
            'country_name' => 'East Timor(+670)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 63,
            'country_code' => 593,
            'country_name' => 'Ecuador (+593)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 64,
            'country_code' => 20,
            'country_name' => 'Egypt (+20)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 65,
            'country_code' => 503,
            'country_name' => 'El Salvador (+503)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 66,
            'country_code' => 240,
            'country_name' => 'Equatorial Guinea (+240)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 67,
            'country_code' => 291,
            'country_name' => 'Eritrea (+291)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 68,
            'country_code' => 372,
            'country_name' => 'Estonia (+372)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 69,
            'country_code' => 251,
            'country_name' => 'Ethiopia (+251)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 70,
            'country_code' => 672,
            'country_name' => 'External Territories of Australia(+672)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 71,
            'country_code' => 500,
            'country_name' => 'Falkland Islands (+500)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 72,
            'country_code' => 298,
            'country_name' => 'Faroe Islands (+298)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 73,
            'country_code' => 679,
            'country_name' => 'Fiji (+679)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 74,
            'country_code' => 358,
            'country_name' => 'Finland (+358)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 75,
            'country_code' => 33,
            'country_name' => 'France (+33)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 76,
            'country_code' => 594,
            'country_name' => 'French Guiana (+594)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 77,
            'country_code' => 689,
            'country_name' => 'French Polynesia (+689)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 78,
            'country_code' => 262,
            'country_name' => 'French Southern Territories(262)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 79,
            'country_code' => 241,
            'country_name' => 'Gabon (+241)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 80,
            'country_code' => 220,
            'country_name' => 'Gambia (+220)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 81,
            'country_code' => 7880,
            'country_name' => 'Georgia (+7880)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 82,
            'country_code' => 49,
            'country_name' => 'Germany (+49)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 83,
            'country_code' => 233,
            'country_name' => 'Ghana (+233)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 84,
            'country_code' => 350,
            'country_name' => 'Gibraltar (+350)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 85,
            'country_code' => 30,
            'country_name' => 'Greece (+30)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 86,
            'country_code' => 299,
            'country_name' => 'Greenland (+299)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 87,
            'country_code' => 1473,
            'country_name' => 'Grenada (+1473)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 88,
            'country_code' => 590,
            'country_name' => 'Guadeloupe (+590)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 89,
            'country_code' => 671,
            'country_name' => 'Guam (+671)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 90,
            'country_code' => 502,
            'country_name' => 'Guatemala (+502)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 91,
            'country_code' => 44,
            'country_name' => 'Guernsey and Alderney(44 1481)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 92,
            'country_code' => 224,
            'country_name' => 'Guinea (+224)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 93,
            'country_code' => 245,
            'country_name' => 'Guinea - Bissau (+245)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 94,
            'country_code' => 592,
            'country_name' => 'Guyana (+592)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 95,
            'country_code' => 509,
            'country_name' => 'Haiti (+509)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 96,
            'country_code' => 672,
            'country_name' => 'Heard and McDonald Islands',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 97,
            'country_code' => 504,
            'country_name' => 'Honduras (+504)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 98,
            'country_code' => 852,
            'country_name' => 'Hong Kong (+852)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 99,
            'country_code' => 36,
            'country_name' => 'Hungary (+36)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 100,
            'country_code' => 354,
            'country_name' => 'Iceland (+354)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 101,
            'country_code' => 91,
            'country_name' => 'India (+91)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 102,
            'country_code' => 62,
            'country_name' => 'Indonesia (+62)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 103,
            'country_code' => 98,
            'country_name' => 'Iran (+98)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 104,
            'country_code' => 964,
            'country_name' => 'Iraq (+964)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 105,
            'country_code' => 353,
            'country_name' => 'Ireland (+353)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 106,
            'country_code' => 972,
            'country_name' => 'Israel (+972)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 107,
            'country_code' => 39,
            'country_name' => 'Italy (+39)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 108,
            'country_code' => 1876,
            'country_name' => 'Jamaica (+1876)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 109,
            'country_code' => 81,
            'country_name' => 'Japan (+81)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 110,
            'country_code' => 44,
            'country_name' => 'Jersey(+44)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 111,
            'country_code' => 962,
            'country_name' => 'Jordan (+962)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 112,
            'country_code' => 7,
            'country_name' => 'Kazakhstan (+7)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 113,
            'country_code' => 254,
            'country_name' => 'Kenya (+254)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 114,
            'country_code' => 686,
            'country_name' => 'Kiribati (+686)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 115,
            'country_code' => 850,
            'country_name' => 'Korea North (+850)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 116,
            'country_code' => 82,
            'country_name' => 'Korea South (+82)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 117,
            'country_code' => 965,
            'country_name' => 'Kuwait (+965)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 118,
            'country_code' => 996,
            'country_name' => 'Kyrgyzstan (+996)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 119,
            'country_code' => 856,
            'country_name' => 'Laos (+856)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 120,
            'country_code' => 371,
            'country_name' => 'Latvia (+371)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 121,
            'country_code' => 961,
            'country_name' => 'Lebanon (+961)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 122,
            'country_code' => 266,
            'country_name' => 'Lesotho (+266)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 123,
            'country_code' => 231,
            'country_name' => 'Liberia (+231)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 124,
            'country_code' => 218,
            'country_name' => 'Libya (+218)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 125,
            'country_code' => 417,
            'country_name' => 'Liechtenstein (+417)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 126,
            'country_code' => 370,
            'country_name' => 'Lithuania (+370)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 127,
            'country_code' => 352,
            'country_name' => 'Luxembourg (+352)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 128,
            'country_code' => 853,
            'country_name' => 'Macao (+853)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 129,
            'country_code' => 389,
            'country_name' => 'Macedonia (+389)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 130,
            'country_code' => 261,
            'country_name' => 'Madagascar (+261)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 131,
            'country_code' => 265,
            'country_name' => 'Malawi (+265)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 132,
            'country_code' => 60,
            'country_name' => 'Malaysia (+60)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 133,
            'country_code' => 960,
            'country_name' => 'Maldives (+960)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 134,
            'country_code' => 223,
            'country_name' => 'Mali (+223)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 135,
            'country_code' => 356,
            'country_name' => 'Malta (+356)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 136,
            'country_code' => 44,
            'country_name' => 'Man (Isle of)(+44)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 137,
            'country_code' => 692,
            'country_name' => 'Marshall Islands (+692)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 138,
            'country_code' => 596,
            'country_name' => 'Martinique (+596)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 139,
            'country_code' => 222,
            'country_name' => 'Mauritania (+222)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 140,
            'country_code' => 230,
            'country_name' => 'Mauritius(+230)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 141,
            'country_code' => 269,
            'country_name' => 'Mayotte (+269)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 142,
            'country_code' => 52,
            'country_name' => 'Mexico (+52)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 143,
            'country_code' => 691,
            'country_name' => 'Micronesia (+691)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 144,
            'country_code' => 373,
            'country_name' => 'Moldova (+373)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 145,
            'country_code' => 377,
            'country_name' => 'Monaco (+377)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 146,
            'country_code' => 976,
            'country_name' => 'Mongolia (+976)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 147,
            'country_code' => 1664,
            'country_name' => 'Montserrat (+1664)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 148,
            'country_code' => 212,
            'country_name' => 'Morocco (+212)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 149,
            'country_code' => 258,
            'country_name' => 'Mozambique (+258)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 150,
            'country_code' => 95,
            'country_name' => 'Myanmar(+95)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 151,
            'country_code' => 264,
            'country_name' => 'Namibia (+264)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 152,
            'country_code' => 674,
            'country_name' => 'Nauru (+674)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 153,
            'country_code' => 977,
            'country_name' => 'Nepal (+977)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 154,
            'country_code' => 599,
            'country_name' => 'Netherlands Antilles(+599)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 155,
            'country_code' => 31,
            'country_name' => 'Netherlands (+31)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 156,
            'country_code' => 687,
            'country_name' => 'New Caledonia (+687)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 157,
            'country_code' => 64,
            'country_name' => 'New Zealand (+64)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 158,
            'country_code' => 505,
            'country_name' => 'Nicaragua (+505)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 159,
            'country_code' => 227,
            'country_name' => 'Niger (+227)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 160,
            'country_code' => 234,
            'country_name' => 'Nigeria (+234)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 161,
            'country_code' => 683,
            'country_name' => 'Niue (+683)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 162,
            'country_code' => 672,
            'country_name' => 'Norfolk Islands (+672)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 163,
            'country_code' => 1,
            'country_name' => 'Northern Mariana Islands(+1)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 164,
            'country_code' => 47,
            'country_name' => 'Norway (+47)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 165,
            'country_code' => 968,
            'country_name' => 'Oman (+968)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 166,
            'country_code' => 92,
            'country_name' => 'Pakistan(+92)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 167,
            'country_code' => 680,
            'country_name' => 'Palau (+680)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 168,
            'country_code' => 970,
            'country_name' => 'Palestinian Territory Occupied(+970)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 169,
            'country_code' => 507,
            'country_name' => 'Panama (+507)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 170,
            'country_code' => 675,
            'country_name' => 'Papua New Guinea (+675)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 171,
            'country_code' => 595,
            'country_name' => 'Paraguay (+595)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 172,
            'country_code' => 51,
            'country_name' => 'Peru (+51)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 173,
            'country_code' => 63,
            'country_name' => 'Philippines (+63)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 174,
            'country_code' => 64,
            'country_name' => 'Pitcairn Island(+64)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 175,
            'country_code' => 48,
            'country_name' => 'Poland (+48)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 176,
            'country_code' => 351,
            'country_name' => 'Portugal (+351)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 177,
            'country_code' => 1787,
            'country_name' => 'Puerto Rico (+1787)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 178,
            'country_code' => 974,
            'country_name' => 'Qatar (+974)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 179,
            'country_code' => 262,
            'country_name' => 'Reunion (+262)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 180,
            'country_code' => 40,
            'country_name' => 'Romania (+40)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 181,
            'country_code' => 7,
            'country_name' => 'Russia (+7)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 182,
            'country_code' => 250,
            'country_name' => 'Rwanda (+250)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 183,
            'country_code' => 290,
            'country_name' => 'St. Helena (+290)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 184,
            'country_code' => 1869,
            'country_name' => 'St. Kitts (+1869)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 185,
            'country_code' => 1758,
            'country_name' => 'Saint Lucia(+1758)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 186,
            'country_code' => 508,
            'country_name' => 'Saint Pierre and Miquelon(+508)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 187,
            'country_code' => 1784,
            'country_name' => 'Saint Vincent And The Grenadines(+1784)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 188,
            'country_code' => 685,
            'country_name' => 'Samoa(+685)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 189,
            'country_code' => 378,
            'country_name' => 'San Marino (+378)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 190,
            'country_code' => 239,
            'country_name' => 'Sao Tome & Principe (+239)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 191,
            'country_code' => 966,
            'country_name' => 'Saudi Arabia (+966)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 192,
            'country_code' => 221,
            'country_name' => 'Senegal (+221)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 193,
            'country_code' => 381,
            'country_name' => 'Serbia(+381)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 194,
            'country_code' => 248,
            'country_name' => 'Seychelles (+248)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 195,
            'country_code' => 232,
            'country_name' => 'Sierra Leone (+232)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 196,
            'country_code' => 65,
            'country_name' => 'Singapore (+65)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 197,
            'country_code' => 421,
            'country_name' => 'Slovak Republic (+421)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 198,
            'country_code' => 386,
            'country_name' => 'Slovenia (+386)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 199,
            'country_code' => 44,
            'country_name' => 'Smaller Territories of the UK',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 200,
            'country_code' => 677,
            'country_name' => 'Solomon Islands (+677)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 201,
            'country_code' => 252,
            'country_name' => 'Somalia (+252)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 202,
            'country_code' => 27,
            'country_name' => 'South Africa (+27)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 203,
            'country_code' => 500,
            'country_name' => 'South Georgia(+500)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 204,
            'country_code' => 211,
            'country_name' => 'South Sudan(+211)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 205,
            'country_code' => 34,
            'country_name' => 'Spain (+34)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 206,
            'country_code' => 94,
            'country_name' => 'Sri Lanka (+94)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 207,
            'country_code' => 249,
            'country_name' => 'Sudan (+249)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 208,
            'country_code' => 597,
            'country_name' => 'Suriname (+597)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 209,
            'country_code' => 47,
            'country_name' => 'Svalbard And Jan Mayen Islands(+41)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 210,
            'country_code' => 268,
            'country_name' => 'Swaziland (+268)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 211,
            'country_code' => 46,
            'country_name' => 'Sweden (+46)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 212,
            'country_code' => 41,
            'country_name' => 'Switzerland (+41)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 213,
            'country_code' => 963,
            'country_name' => 'Syria(+963)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 214,
            'country_code' => 886,
            'country_name' => 'Taiwan (+886)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 215,
            'country_code' => 7,
            'country_name' => 'Tajikstan (+7)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 216,
            'country_code' => 255,
            'country_name' => 'Tanzania(+255)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 217,
            'country_code' => 66,
            'country_name' => 'Thailand (+66)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 218,
            'country_code' => 228,
            'country_name' => 'Togo (+228)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 219,
            'country_code' => 690,
            'country_name' => 'Tokelau(+690)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 220,
            'country_code' => 676,
            'country_name' => 'Tonga (+676)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 221,
            'country_code' => 1868,
            'country_name' => 'Trinidad & Tobago (+1868)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 222,
            'country_code' => 216,
            'country_name' => 'Tunisia (+216)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 223,
            'country_code' => 90,
            'country_name' => 'Turkey (+90)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 224,
            'country_code' => 7,
            'country_name' => 'Turkmenistan (+7)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 225,
            'country_code' => 1649,
            'country_name' => 'Turks & Caicos Islands (+1649)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 226,
            'country_code' => 688,
            'country_name' => 'Tuvalu (+688)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 227,
            'country_code' => 256,
            'country_name' => 'Uganda (+256)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 228,
            'country_code' => 380,
            'country_name' => 'Ukraine (+380)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 229,
            'country_code' => 971,
            'country_name' => 'United Arab Emirates (+971)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 230,
            'country_code' => 44,
            'country_name' => 'UK (+44)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 231,
            'country_code' => 1,
            'country_name' => 'USA (+1)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 232,
            'country_code' => 246,
            'country_name' => 'United States Minor Outlying Islands(+246)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 233,
            'country_code' => 598,
            'country_name' => 'Uruguay (+598)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 234,
            'country_code' => 7,
            'country_name' => 'Uzbekistan (+7)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 235,
            'country_code' => 678,
            'country_name' => 'Vanuatu (+678)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 236,
            'country_code' => 379,
            'country_name' => 'Vatican City (+379)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 237,
            'country_code' => 58,
            'country_name' => 'Venezuela (+58)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 238,
            'country_code' => 84,
            'country_name' => 'Vietnam (+84)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 239,
            'country_code' => 84,
            'country_name' => 'Virgin Islands - British (+1284)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 240,
            'country_code' => 84,
            'country_name' => 'Virgin Islands - US (+1340)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 241,
            'country_code' => 681,
            'country_name' => 'Wallis & Futuna (+681)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 242,
            'country_code' => 212,
            'country_name' => 'Western Sahara(+212)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 243,
            'country_code' => 969,
            'country_name' => 'Yemen (North)(+969)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 244,
            'country_code' => 38,
            'country_name' => 'Yugoslavia(+38)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 245,
            'country_code' => 260,
            'country_name' => 'Zambia (+260)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 246,
            'country_code' => 263,
            'country_name' => 'Zimbabwe (+263)',
            'status' => 'active'
        ]);

        Country::create([
            'id' => 247,
            'country_code' => 156,
            'country_name' => 'Bahraini  +973',
            'status' => 'active',
            'created_at' => NULL,
            'updated_at' => '2021-07-10 10:03:24'
        ]);
    }
}
